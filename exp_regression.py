...
def draw_picture(features, values, w, method, filename, **kwargs):
    features = features[:, 1:] #discard bias features
    if method == 'bayesian':
        SD = kwargs.get('SD')
        beta = kwargs.get('beta')
        draw_gauss(sin2pix, features, values, w,
                   SD, beta, 'result/'+filename)
    else:
        draw_curve(sin2pix, features, values, w, 'result/'+filename)
